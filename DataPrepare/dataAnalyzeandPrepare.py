...
pd.set_option('display.unicode.east_asian_width', True)  # 修改对齐设定，使得数据输出时更好看
...
# %%
# 如果有重复值则去重
loc = pd.DataFrame(dt_new[dt_new.duplicated(keep=False) == True].index.tolist(
)).transpose()  # 找到重复值的数据地址，并转置显示，设定keep=False用于输出所有重复值的位置（如果不加，只输出重复值最后出现的位置）
display(loc)  # 获取重复值位置
...
dt_new['status_published'] = pd.to_datetime(dt_new['status_published'],
                                            format='%m/%d/%Y %H:%M',
                                            errors='coerce')
dt_new['status_published'] = dt_new['status_published'].astype('int64') // 1e9
