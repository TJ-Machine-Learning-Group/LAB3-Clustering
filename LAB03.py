...
if __name__ == "__main__":
    ...
    data = GetPCAData(csv_path=csv_path, n_components=10)
    print(data)
    ...
    colors = [
        ...
        "#FFF000", "#00FFF0", "#F000FF", "#F00000", "#00F000", "#0000F0",
        "#F00F00", "#000000"
    ]
    ...
    indexs = [
        "number of clusters", "num of noise points", "Silhouette Coefficient",
        "Calinski-Harabaz Index", "Davies-Bouldin Index", "all"
    ]
    size = len(indexs)
    wb = Workbook()
    ws = wb.active
    ...
    for i, j in enumerate(list(labels.keys())):
        ws.merge_cells(start_row=i * size + 2,
                       end_row=(i + 1) * size + 1,
                       start_column=1,
                       end_column=1)
        ...
        for s, t in enumerate(indexs):
            ...
            ws.cell(row=i * size + 2 + s,
                    column=2).alignment = alignment_center
    for j in range(2, 9):
        ...
        for i, k in enumerate(labels.items()):
            ...
            for s in range(len(eval_data)):
                ws.cell(row=i * size + 2 + s,
                        column=j + 1).value = eval_data[s]
                ws.cell(row=i * size + 2 + s,
                        column=j + 1).alignment = alignment_center
    wb.save('test.xlsx')
